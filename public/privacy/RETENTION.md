...
| `deviceIdsByUid/{uid}_{date}` | Device-rotation ceiling (hashes an account presented that day) | 3 days | Self-expiring doc ids, and `deleteByUid` on account deletion | Erased by name on deletion — keyed by uid. |
| `deletionResidue/{uid}` | Ops breadcrumb written ONLY when part of an erasure fails (uid + which collections survived) | 30 days | **TTL policy required** on `expireAt` | **Action required:** enable Firestore TTL on `deletionResidue.expireAt`. [audit-H13] Undeclared until now. A user told their data is permanently erased could hold a uid-keyed record for 30 days; it exists precisely because that erasure did NOT complete, so it must be disclosed rather than treated as internal. |
